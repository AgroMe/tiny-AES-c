...
const PROGMEM uint8_t sbox[256] = {
...
const PROGMEM uint8_t rsbox[256] = {
...
{
  return pgm_read_byte_near(sbox + num);
}

static uint8_t getSBoxInvert(uint8_t num)
{
  return pgm_read_byte_near(rsbox + num);
}
...
static void KeyExpansion(void)
{
  ...
  for (; i < Nb * (Nr + 1); ++i)
  {
    ...
    if (i % Nk == 0)
    {
      ...
      tempa[0] =  tempa[0] ^ pgm_read_byte_near(Rcon + i/Nk);
